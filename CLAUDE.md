...
- **`agent-manager.ts`** — Core logic. `AgentManager` class handles all agent CRUD, task queuing (inbox.jsonl), event emission (outbox.jsonl), and peer broadcast. Uses file system for all persistence.
- **`tmux.ts`** — Thin wrappers around `tmux` CLI commands: `splitPane`, `sendKeys`, `killPane`, `killSession`, etc.
- **`types.ts`** — TypeScript interfaces: `Agent`, `Task`, `AgentEvent`, `CreateAgentParams`.
...
.agents/sessions/<session_id>/
  broadcast.jsonl                          # All events across agents
  agents/<agent_id>/
    meta.json                              # Agent state snapshot
    inbox.jsonl                            # Task queue (append-only, cursor-based)
    outbox.jsonl                           # Agent events (append-only)
...
The inception prompt instructs the sub-agent to loop on `wait_for_command` (cursor-based, 30s timeout), call `emit_event` with results, and repeat.

`wait_for_command` polls `inbox.jsonl` every 500ms using a line index (cursor). `emitEvent` appends to both `outbox.jsonl` and `broadcast.jsonl`, and also fans out to every other agent's `inbox.jsonl`.
...
- **`tests/mocks/mock-gemini.js`** — Node.js script that simulates a Gemini sub-agent: emits `agent_ready` on startup, polls `inbox.jsonl` and emits `task_completed` for each task.
